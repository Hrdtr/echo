        ...
        :links="[
          {
            ...
            icon: props.connected ? 'i-heroicons-signal-slash' : 'i-heroicons-signal',
            click: (e: any) => {
              e.preventDefault()
              e.stopPropagation()
              if (props.connectionPending) return
              emit('toggleConnection')
            },
          },
        ]"
        :ui="{ font: 'font-normal', width: 'min-w-36' }"
      >
        <template #icon>
          <span class="hidden" />
        </template>
        <template #badge="{ link }">
          <UIcon
            :name="props.connectionPending ? 'i-heroicons-arrow-path-20-solid' : link.icon"
            :class="props.connectionPending ? 'animate-spin' : ''"
